...
# NOTE use np.random.choice or rng.integer here?
...
# TODO: Look to vectorize this god awful for-loop away!
for j in range(sets_sims):
    for i in range(num_simulations):
        ...
        while remaining_steps_til_expiry > 0:
            ...
            portfolio["share_PnL"] = portfolio[
                "share_price_change"
            ] * portfolio["share_position"].shift(1)
            portfolio["option_PnL"] = (
                100
                * portfolio["option_price_change"]
                * portfolio["option_position"].shift(1)
            )

            # compute the cumulative PnL over all steps
            portfolio["cumulative_share_PnL"] = portfolio["share_PnL"].cumsum()
            portfolio["cumulative_option_PnL"] = portfolio[
                "option_PnL"
            ].cumsum()
            portfolio["cumulative_portfolio_PnL"] = (
                portfolio["cumulative_share_PnL"]
                + portfolio["cumulative_option_PnL"]
            )
            cumulative_portfolio_profit = portfolio[
                "cumulative_portfolio_PnL"
            ].iloc[-1]
            ...
            if current_step <= total_steps_dte:
                path = {
                    ...
                    "callput": callput,
                    "share_price": stock_price,
                    "cumulative_portfolio_PnL": round(
                        cumulative_portfolio_profit, 0
                    ),
                }
        ...
        simulation = {
            ...
            "stock_return": round(
                (stock_price / portfolio["share_price"].iloc[0] - 1), 3
            ),
            "delta_hedged_PnL": round(delta_hedged_profit, 0),
        }

        df_simulation = pd.DataFrame([simulation])
        simulation_table = pd.concat(
            [simulation_table, df_simulation], ignore_index=True
        )
    # %%
    ...
    sim_summary = path_table[
        (path_table["sim_number"] == j + 1)
        & (path_table["current_step"] == total_steps_dte)
    ]
    ...
    sets_of_sims_table = pd.concat(
        [sets_of_sims_table, df_log_entry], ignore_index=True
    )
    ...
    print(
        f"St_dev_scaled_to_initial_option_premium: {round(0.01 * round(sim_std_dev, 0) / portfolio['option_price'].iloc[0], 3)}"
    )
    print("---" * 40)
# %%
# First figure
...
sns.histplot(
    simulation_table["terminal_price"], kde=True, ax=ax1[0], stat="percent"
)
...
print(f"---" * 40)
print(
    f"Mean profit across all sets of sims: {round(sets_of_sims_table['mean_PnL'].mean(), 1)}"
)
print(
    f"Standard Dev of profit across all sets of sims: {round(sets_of_sims_table['sim_total_PnL'].std(), 1)}"
)
print(f"---" * 40)
print(f"Mean_stock_return: {round(simulation_table['stock_return'].mean(), 3)}")
print(
    f"Modal_stock_return: {round(simulation_table['stock_return'].mode(), 3)}"
)
...
# Theoretical
distribution = _theo_binom_distribution(
    total_steps_dte, up_return, up_prob, init_stock_price
)
distro_df = pd.DataFrame(
    distribution, columns=["terminal_price", "theo_frequency"]
)
distro_df["theo_frequency"] = distro_df["theo_frequency"].round(4)

# Actual
terminal_price_proportions = simulation_table["terminal_price"].value_counts(
    normalize=True
)
...
terminal_price_proportions["actual_frequency"] = terminal_price_proportions[
    "actual_frequency"
].round(4)
...
terminal_price_proportions["terminal_price"] = terminal_price_proportions[
    "terminal_price"
].round(_decimals)

merged_df = distro_df.merge(
    terminal_price_proportions, on="terminal_price", how="left"
)
merged_df["actual-theo"] = (
    merged_df["actual_frequency"] - merged_df["theo_frequency"]
)
...
ax2[0].bar(
    [p + width for p in position],
    merged_df["theo_frequency"],
    width=width,
...
ax2[0].set_xticklabels(
    merged_df["terminal_price"].values, rotation=45, ha="right"
)
ax2[0].set_title(
    "Comparison of Actual & Theoretical Frequencies for each Terminal Price"
)
...
ax2[0].grid(axis="y")
ax2[0].yaxis.set_major_formatter(
    mticker.PercentFormatter(1.0, decimals=0)
)  # format as percent with 1 decminal
...
ax2[1].grid(axis="y")
ax2[1].yaxis.set_major_formatter(
    mticker.PercentFormatter(1.0, decimals=2)
